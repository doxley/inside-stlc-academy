...
- **Ignoring informal adoption** because no one has "approved" it — the risk is already live whether you look at it or not`,
      ...
      enhancements: {
        industryStory: `It's a common pattern, and it almost never arrives as an announcement. Consider a QA function where a leader learns — through a passing remark in a one-to-one, not a policy breach report — that a couple of testers have quietly been pasting failing test code and awkward tickets into a free public chatbot to get themselves unstuck. The honest first instinct in that moment is usually a jolt of alarm, and the reflex that follows is to shut it down. The leaders who handle it well resist that reflex, because they notice the more important signal underneath: people are using the tool precisely because it helps them, which means a ban will not stop the behaviour, only push it onto personal devices and accounts where it cannot be seen or controlled at all. So instead of a prohibition or a shrug, the productive move is to treat the discovery as intelligence rather than a crime — find out what people are actually doing and why, plot those uses by value and risk, and offer a safe, sanctioned path for the ones worth keeping. The moment you learn AI is already in the building is not the moment to panic; it is the moment you finally have something concrete to govern.`,
        visualAid: {
          type: 'matrix',
...
- **Judging design by volume of ideas** ("it generated forty cases!") instead of by whether the risky, context-specific cases are covered`,
      ...
      enhancements: {
        ...
        miniChallenge: `Aisha, Northstar's newest tester, still ramping, tells you she has started generating all her test designs with AI because "it's faster and finds more." She is pleased. In two or three sentences, decide how you coach her without discouraging the initiative.`,
        modelAnswer: `## Example
I'd validate the instinct and then reshape the habit. "Speed and breadth are real wins — keep using it. But while you're learning this system, I want you to design the first pass yourself, *then* run the AI to catch what you missed, not the other way round." For a tester still building judgement, AI-first design robs her of the reps that make a great tester; AI-second turns the tool into a coach that shows her her own blind spots. Same tool, sequenced to build her craft instead of substituting for it — and I'd revisit the balance as she becomes more senior.`,
      },
    },
...
- **Playing lawyer or security officer** — inventing data-protection rules instead of anchoring in the organisation's real policy and bringing the experts in as partners`,
      ...
      enhancements: {
        industryStory: `Time and again, shadow AI is handled in one of two ways, and the contrast between them is the whole lesson. Picture an organisation that reacts to the discovery with a blanket ban — an all-staff email forbidding any AI tool overnight, issued to look decisive. The usage does not stop; it migrates onto personal phones and personal accounts, where leadership now has no visibility, no data controls and no idea what is being pasted where. A manageable, observable risk has been converted into an invisible one, and nothing has been learned in the process. Now picture a function that responds the other way: it acknowledges the usage openly, brings the security officer in as a co-author rather than a police officer, sanctions a single governed tool to replace the public ones, and writes a short list of approved uses. The valuable work carries on through a safe door, and the risky habits fade because there is finally an easier, sanctioned alternative to reach for. The pattern that keeps repeating is that a ban feels responsible while quietly making things worse, whereas getting ahead of the usage — earlier and more calmly than instinct wants — is what actually shrinks the risk, and turns a nervous security team from a blocker into an ally almost as a side effect.`,
        visualAid: {
          type: 'flow',
