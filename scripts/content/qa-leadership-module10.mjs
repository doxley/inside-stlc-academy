...
- **Making the decision yourself** — even a well-reasoned "we will ship" takes on accountability that belongs to the business; recommend, then hand over the call`,
      ...
      enhancements: {
        industryStory: `[DAVID INPUT REQUIRED: Add a short, authentic story of a real high-pressure release deadline you faced or advised on — ideally one where the difference between a flat "no" and a structured risk-and-recommendation approach changed the outcome, or where you learned the hard way that refusing simply got QA overruled. What specifically did the leadership response contain, and how did the accountable stakeholder react?]`,
        visualAid: {
          type: 'flow',
...
- **Framing resource needs as capacity** ("we need more testers") rather than as risk-and-return, which is the language that actually gets QA funded`,
      ...
      enhancements: {
        industryStory: `[DAVID INPUT REQUIRED: Add a short, authentic example from your experience of an executive or board quality conversation — ideally a contrast between a QA update that landed (framed in risk, money and outcomes) and one that fell flat (test-case counts and activity), and what the difference did to QA's standing and funding. If possible, include the specific phrase or framing that made an executive genuinely pay attention.]`,
        visualAid: {
          type: 'comparison',
