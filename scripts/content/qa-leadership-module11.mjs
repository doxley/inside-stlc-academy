...
- **Having the conversation and never following up**, teaching the team your standards don't mean anything`,
      ...
      enhancements: {
        industryStory: `Picture a manager who walks into a performance conversation without preparing — no examples pulled, no check that the standard was ever set, just a vague sense that a tester "needs to pick things up." It goes exactly as you'd expect: the tester asks "like when, though?", the manager can't answer cleanly, the whole thing collapses into generalities, and the person leaves defensive and none the wiser. Contrast the same manager one conversation later, having done the work — three specific sign-offs written down, the agreed expectation confirmed, and a genuine goal of understanding rather than winning. This time the message lands, the tester actually names the cause, and a fixable plan comes out of it. The lesson that tends to stick is about the boundary: the well-prepared conversation stayed a leadership conversation precisely because it surfaced a skills cause early and fairly — had the manager improvised, quietly decided the verdict in advance, and started "building a case," it would have drifted toward a quasi-formal footing with none of the fairness or due process a real one requires. Prepare properly and most of these never need to go near HR; freelance them and you manufacture the exact unfairness that should have been handed to HR in the first place.`,
        visualAid: {
          type: 'timeline',
...
- **Confusing "I find this person difficult" with underperformance** — dislike is not a performance standard`,
      ...
      enhancements: {
        industryStory: `It's a common pattern: a tester's output drops off, the manager reads it as laziness or a bad attitude, and the instinct is to "start the process." Picture that manager pausing to diagnose before acting — and discovering the drop had nothing to do with will at all. In one version it turns out to be a skills gap the system itself created: the codebase moved toward APIs and integration the tester was never trained on, and what looked like carelessness was someone quietly out of their depth and too proud to say so. In another it's something personal — a caring responsibility or a health matter at home — that no performance plan could ever have fixed and that a manager has no business navigating alone. Either way the honest diagnosis completely changes the response: development and time for the skills case, humanity and early HR involvement for the personal one. And where a case genuinely does prove to be a will problem persisting after real, cause-matched support, the same discipline applies at the other end — the moment it looks as though it could lead to a warning or an exit, it stops being a conversation you own and becomes an HR-led process, with the earlier stages handled fairly enough that nothing about it comes as a surprise. The leaders who get this wrong are almost always the ones who decided the outcome before they ever understood the cause.`,
        visualAid: {
          type: 'flow',
